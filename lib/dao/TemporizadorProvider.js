  ...
  _logs = logs;
    ...
    if (config.MONGO.Habilitado == "true") {
      ...
      var TemporizadorSchema = new Schema({
          IdTemporizador : { type: Number, index: { unique: true }, required : true }
    ...
    if (config.NEDB.Habilitado == "true"){
      ...
      db.Temporizador = new Datastore({ filename: config.NEDB.Path + 'Temporizador.db', autoload:true});
	...
	if (_provider.NEDB) {
        ...
        var TemporizadorModel = new Models();
	      ...
	      db.Temporizador.find(filters, function (err, docs) {
	        ...
	        if (err) {
	           if (_DEBUG) {
               console.log("<NEDB> TemporizadorProvider.Find : Error al leer async en mettodo Find() error : " + error);
             }
	           return callback(err, lstModels);
	        }
	        else
	        {
	          ...
	          return callback(null, TemporizadorModel.Objeto());
	        }
	      });
	  }
