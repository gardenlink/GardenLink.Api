      ...
      if (conf == "MONGO")
      {
        if (Configuracion[conf].Habilitado == "true")
        {
          ...
          var options = { server: { socketOptions: { keepAlive: 300000, connectTimeoutMS: 30000 },
             ...
             try {
          mongoose.connect('mongodb://'+Configuracion[conf].UserName+':'+Configuracion[conf].Password+'@'+ Configuracion[conf].Servidor + ':' + Configuracion[conf].Puerto+ '/'+ Configuracion[conf].BaseDatos, options);
          } catch(e) {
...
method.Cache = function(enabled,callback) {
	if (enabled && enabled == true && datosConsolidados && datosConsolidados["Dispositivos"] && datosConsolidados["Dispositivos"].length > 0) {
...
function MotorExists(lst, value) {
	...
	}, function(error) { if (_DEBUG) console.log("DataProvider.MotorExists : Error al leer async en metodo MotorExists error : " + error); return retorno; });
...
function SensorExists(lst, value) {
	...
	}, function(error) { if (_DEBUG) console.log("DataProvider.SensorExists : Error al leer async en metodo SensorExists error : " + error); return retorno; });
...
function DeviceExists(lst, value) {
	...
	}, function(error) { if (_DEBUG) console.log("DataProvider.DeviceExists : Error al leer async en mettodo DeviceExists() error : " + error);
						return retorno;
