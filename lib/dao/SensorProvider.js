  ...
  _logs = logs;
    ...
    if (config.MONGO.Habilitado == "true") {
      ...
      var SensorSchema = new Schema({
         IdSensor : { type: Number, index: { unique: true }, required : true }
    ...
    if (config.NEDB.Habilitado == "true"){
      ...
      db.Sensor = new Datastore({ filename: config.NEDB.Path + 'Sensor.db', autoload:true});
...
method.Save = function(IdSensor, IdDispositivo, Descripcion, MarcaModelo, Tipo,Pin,EsPinAnalogo,Pin2,EsPinAnalogo2,Pin3,EsPinAnalogo3,Habilitado,callback) {
    ...
    if (_provider.MONGO) {
      ...
      MongoModels.findOne({IdSensor : sensorModel.Objeto().IdSensor}, function (err, data) {
        ...
        if (!data || data.length == 0) {
	          ...
	          objMongo.save(sensorModel.Objeto(), function(err, data) {
		          if (_DEBUG)
		            console.log("<MONGO> SensorProvider.Save : inserto nuevo objeto, detalle : " + JSON.stringify(sensorModel.Objeto()));
        ...
        else
        {
          MongoModels.update({ _id: data._id }, { $set: {
              ...
              Pi2: sensorModel.Objeto().Pin2,
              EsPinAnalogo2: sensorModel.Objeto().EsPinAnalogo2,
              Pin3: sensorModel.Objeto().Pin3,
              EsPinAnalogo3: sensorModel.Objeto().EsPinAnalogo3,
              ...
              if (err) {
                ...
                if (_DEBUG)
                  console.log("<MONGO> SensorProvider.Save : Remplazados: " + JSON.stringify(numReplaced));
                if(callback)
      ...
      else
      {
        ...
        if (callback)
          return callback("<MONGO> SensorProvider.Save : No se graba objeto debido a que no pasa la validacion de integridad", null);
      }
    }
 ...
 if (_provider.MONGO) {
  ...
  if (_provider.NEDB) {
      ...
      db.Sensor.find(filters, function (err, docs) {
        ...
        if (err) {
           if (_DEBUG) console.log("<NEDB> SensorProvider.GetCollection : Error al leer async en mettodo GetAll() error : " + error);
           return callback(err, lstModels);
        }
        else
        {
          ...
          }, function(error) { if (_DEBUG) console.log("<NEDB> SensorProvider.GetCollection : Error al leer async en mettodo GetAll() error : " + error); });
          }
          return callback(null, lstModels);
        }
      });
