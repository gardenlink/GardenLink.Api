	       ...
	       //Monitoreo Sensores
			     ...
			     dataProvider.Sensor().GetCollection(filter, function(err, data) {
				    if (data.length > 0) {
				        ...
				        async.each(data, function(item, callback) {
				         ...
				         if (item.Habilitado == "true" || item.Habilitado == true) {
									  ...
									  //Configurar
										console.log("Configurando sensor ID : " + item.IdSensor +
										...
										serviceProvider.Sensor().Configurar(dataProvider, item.IdDispositivo, item.IdSensor, item.Tipo, function(error, data) {
		 					            ...
		 					            if (error)
		 					            {
		 					                ...
		 					                console.log("MonitorSalud.Sensor(): Error al configurar sensor, detalle: " + error);
		 					            }
		 					            else
		 					            {
		 					              //Mido Sensor
				      ...
				      {
				      	console.log("MonitorSalud.Sensor() : No existen sensores asociados al dispositivo [" + item.Id + "]");
				      }
				    });
				    ...
				    if (data.length > 0) {
				      ...
				      {
				      	console.log("MonitorSalud.Motor() No existen Motores asociados al dispositivo [" + item.Id + "]");
				      }
				    });
				    ...
				    if (data.length > 0) {
				      ...
				      {
				      	console.log("MonitorSalud.Relay() : No existen relays asociados al dispositivo [" + item.Id + "]");
				      }
				    });
