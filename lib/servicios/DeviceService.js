...
function LlamarServicio(idDevice, accion, callback)
{
	TraerDatos(_dataProvider, function(error, data) {
	 ...
	 if (_DEBUG)
		_logger.info("DeviceService.LlamarServicio -> Parametros [" + idDevice + "][" + accion + "]");
	  		...
	  		async.each(dispositivos, function(result, callb) {
			  	...
			  	if (result.Id == idDevice)
			  	{
			  		...
			  		if (result.Habilitado && (result.Habilitado == "true" || result.Habilitado == true)) {
						...
						if (accion == "IP") {
							...
							_srv.Leer(topic, function(error, data) {
						            ...
						            deviceModel.Crear(result.Id,
				  			 		...
				  			 		if (_DEBUG)
				  			 			_logger.info("DeviceService.LlamarServicio -> Valor retornado por metodo Leer: " + data);
						...
						_logger.info("DeviceService.LlamarServicio : Intento de trabajar con dispositivo deshabilitado");
						return callback(new Error("DeviceService.LlamarServicio : Intento de trabajar con dispositivo deshabilitado", null));
					}
			  	}

			  }, function(error) { if (error) {
			  		_logger.error("DeviceService.LlamarServicio.LecturaAsyncRelays : error : " + error);
			  		return callback(error, null);
			  		}
			  		else
			  		{
			  			if (!deviceEncontrado)
					  	{
					  		_logger.info("DeviceService.LlamarServicio : Parametros idDispositivo no encontrados en BD");
