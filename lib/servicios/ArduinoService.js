...
function SensorService(dispositivos,logger) {
  ...
  for(var dev in dispositivos)
  {
    if (dispositivos[dev].id == ID_ARDUINO)
    {
      if (dispositivos[dev].habilitado == "true" || dispositivos[dev].habilitado == "TRUE")
      {
        ...
        {
          _logger.info("El dispositivo configurado con ID : " + ID_ARDUINO + " tiene un tipo de tarjeta diferente al configurado");
          break;
        }
      }
      else
      {
        _logger.info("El dispositivo configurado con ID : " + ID_ARDUINO + " esta deshabilitado por config");
        break;
      }
    }
  }
  ...
  if (!dispositivo) {
    _logger.info("SensorService: Dispositivo no encontrado..");
...
function VerificarPin(pin, modo)
{
  if (modo == 'analogo')
  {
    if (!SENSOR_ANALOGO[pin]) {
      _logger.info("El pin no esta disponible para ser ocupado como sensor analogo");
  ...
  else if (modo == 'digital')
  {
     if (!SENSOR_DIGITAL[pin]) {
      _logger.info("El pin no esta disponible para ser ocupado como sensor digital");
      throw new Error("El pin no esta disponible para ser ocupado como sensor digital");
    }
  }
  else
  {
    _logger.info("El pin no existe");
