...
function ServiceProvider(dataProvider,config,logger,user) {
  ...
  var srv = new PonteClient(config.mqtt.http_server, config.mqtt.http_port, "demo" , opt);
   ...
   Servicios.Dispositivo = new Librerias.Dispositivo(dataProvider, srv, logger, function(err, data) {
    if (err) _logger.error("ServiceProvider.Constructor.Dispositivo : error : " + err);
   		logger.info("ServiceProvider.Constructor -> Dispositivo Inicializado ");
   });

   Servicios.Sensor = new Librerias.Sensor(dataProvider, srv, logger, function(err, data){
   if (err) _logger.error("ServiceProvider.Constructor.Sensor : error : " + err);
   else
		logger.info("ServiceProvider.Constructor -> Sensor Inicializado ");
	});

   Servicios.Motor = new Librerias.Motor(dataProvider, srv, logger, function(err, data) {
    if (err) _logger.error("ServiceProvider.Constructor.Motor : error : " + err);
   		logger.info("ServiceProvider.Constructor -> Motor Inicializado ");
   });

   Servicios.Relay = new Librerias.Relay(dataProvider, srv, logger, function(err, data) {
    if (err) _logger.error("ServiceProvider.Constructor.Relay : error : " + err);
   		logger.info("ServiceProvider.Constructor -> Relay Inicializado ");
   });
