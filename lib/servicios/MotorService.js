...
{
	if (_DEBUG)
		_logger.info("MotorService.LlamarServicio -> Parametros [" + idDevice + "][" + idMotor + "][" + accion + "]");
	  		...
	  		async.each(motores, function(result, callb) {
			  	...
			  	if (result.IdDispositivo == idDevice && result.IdMotor == idMotor)
			  	{
			  		...
			  		if (result.Habilitado) {
		  				...
		  				switch (accion)
		  				{
		  					...
		  					case "ESTADO":
		  						...
		  						_srv.Leer(topic, function(error, data) {
				  			 		...
				  			 		if (_DEBUG)
				  			 			_logger.info("MotorService.LlamarServicio -> Valor retornado por metodo Leer: " + data);
		  				...
		  				if (_DEBUG)
		  					_logger.info("Motor.LlamarServicio - Params: " + params);
	  					...
	  					if (accion == "AVANZAR" || accion == "RETROCEDER" || accion == "DETENER") {
	  						...
	  						_srv.Publicar(topic, params, function(error, data) {
			  			 		...
			  			 		if (_DEBUG)
			  			 			_logger.info("MotorService.LlamarServicio -> Valor retornado por metodo Publicar: " + data);
						...
						_logger.info("MotorService.LlamarServicio : Intento de trabajar con motor deshabilitado");
						return callback(new Error("MotorService.LlamarServicio : Intento de trabajar con motor deshabilitado", null));
					}
			  	}

			  }, function(error) { if (error) _logger.error("MotorService.LlamarServicio.LecturaAsyncRelays : error : " + error) });

	  		  if (!motorEncontrado) {
	  		  	if (_DEBUG)
	  		  		_logger.info("motor no encontrado");
	  		  		callback(null, "");
	  		  }
