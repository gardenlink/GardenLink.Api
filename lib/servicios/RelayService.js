...
method.Activar = function(dp, idDevice, idRelay, callback) {
	TraerDatos(dp, function(error, data) {
      LlamarServicio(idDevice, LLAMADA_CONFIGURACION, idRelay, CONSULTAR, function(error,data) {
...
method.Desactivar = function(dp, idDevice, idRelay, callback) {
 	TraerDatos(dp, function(error, data) {
      LlamarServicio(idDevice, LLAMADA_CONFIGURACION, idRelay, CONSULTAR, function(error,data) {
...
method.Estado = function(dp, idDevice, idRelay, callback)
{
	TraerDatos(dp, function(error, data) {
      LlamarServicio(idDevice, LLAMADA_CONFIGURACION, idRelay, CONSULTAR, function(error,data) {
			  	...
			  	if (result.IdDispositivo == idDevice && result.IdRelay == idRelay)
			  	{
			  		...
			  		if (result.Habilitado) {
			            ...
			            relayModel.CrearObjeto(result);
			  			...
			  			//REviso si es una llamada de configuracion
					    ...
					    else
					    {
							switch(accion)
			  				...
			  				case ACTIVAR:
			                  if (result.EsInverso) {
								  params = pRelay+ arduino.ModoLlamada(modoLlamada)  +  
								  			arduino.Operaciones("Relay").Apagar + 
								  			idRelay + 
								  			result.Pin + 
								  			arduino.cmd_escape();
			                    ...
			                    params = pRelay+arduino.ModoLlamada(modoLlamada)  + 
			                    arduino.Operaciones("Relay").Encender + 
			                    idRelay + 
			                    result.Pin + 
			                    arduino.cmd_escape();
								activo = "true";
						  	...
						  	case DESACTIVAR:
                  				if (result.EsInverso) {
			  					  params = pRelay+
			  					  			arduino.ModoLlamada(modoLlamada)  + 
			  					  			arduino.Operaciones("Relay").Encender + 
			  					  			idRelay + 
			  					  			result.Pin + 
			  					  			arduino.cmd_escape();
