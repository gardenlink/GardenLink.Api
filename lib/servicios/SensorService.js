...
{
	 if (_DEBUG)
	 	console.log("SensorService.LlamarServicio -> PARAMS : IdDevice [" + idDevice + "] IdSensor [" + idSensor + "] Accion [" + accion + "]");
	...
	var params;
	 async.each(sensores, function(doc, cb) {
	  	if (doc.IdDispositivo == idDevice && doc.IdSensor == idSensor) {
  			...
  			{
  				case "Temperatura":
  					params = arduino.Dispositivos("Sensor") + arduino.ModoLlamada(modoLlamada)  + arduino.Operaciones("Sensor").Temperatura + idSensor+doc.Pin+arduino.cmd_escape();
  					break;

  				case "Humedad":
  					params = arduino.Dispositivos("Sensor")+ arduino.ModoLlamada(modoLlamada) +arduino.Operaciones("Sensor").Humedad + idSensor+doc.Pin+arduino.cmd_escape();
  					break;

  				case "Lluvia":
  					params = arduino.Dispositivos("Sensor")+ arduino.ModoLlamada(modoLlamada) +arduino.Operaciones("Sensor").Lluvia + idSensor+doc.Pin+arduino.cmd_escape();
  					break;

  				case "PH":
  					params = arduino.Dispositivos("Sensor")+ arduino.ModoLlamada(modoLlamada) + arduino.Operaciones("Sensor").PH + idSensor+doc.Pin+arduino.cmd_escape();
  					break;

  				case "EC":
  					params = arduino.Dispositivos("Sensor")+ arduino.ModoLlamada(modoLlamada) + arduino.Operaciones("Sensor").EC + idSensor+doc.Pin+arduino.cmd_escape();
  					break;

  				case "HumedadTierra":
  					params = arduino.Dispositivos("Sensor")+ arduino.ModoLlamada(modoLlamada) + arduino.Operaciones("Sensor").HumedadTierra + idSensor+doc.Pin+arduino.cmd_escape();
  					break;
			...
			if (_DEBUG)
	  			console.log("SensorService.LlamarServicio() - Params: " + params);
        ...
        if (modoLlamada == LLAMADA_CONFIGURACION) {
          ...
          _srv.Publicar(topic, params, function(error, data) {
    			 		...
    			 		if (_DEBUG)
    			 			console.dir("SensorService.LlamarServicio -> Valor retornado por metodo Publicar: " + data);
        ...
        else
        {
	  		...
	  		else
	  		{
	  			if (!sensorEncontrado)
					  	{
					  		console.log("SensorService.LlamarServicio : Parametros idSensor / idDispositivo no encontrados en BD");
